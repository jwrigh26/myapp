...
export function LearnDrawer() {
  ...
  const isMathRoute = !!matchRoute({
    to: learnPaths.math,
    fuzzy: true, // This makes it match child routes too
  });

  const isDsaRoute = !!matchRoute({
    to: learnPaths.dsa,
    fuzzy: true,
  });

  const isAiRoute = !!matchRoute({
    to: learnPaths.ai,
    fuzzy: true,
  });

  // These can stay the same since they're just filtering static route data
  const mathRoutes = filterRoutes(router, learnPaths.math);
  const dsaRoutes = filterRoutes(router, learnPaths.dsa);
  const aiRoutes = filterRoutes(router, learnPaths.ai);

  return (
    <MiniVariantDrawer width={256} drawerKey="learn-drawer">
      <PostCategory
        title="Math"
        icon={mdiCalculator}
        isActive={isMathRoute}
        routes={mathRoutes}
        categoryPath={learnPaths.math + '/'}
      />
...
function CollapsedCategoryIcon({
  ...
  const handleCategoryClick = () => {
    // Generate section keys dynamically from learnPaths
    const allSectionKeys = Object.keys(learnPaths).map((pathKey) => {
    ...
    if (title === 'Math') {
      categoryBasePath = learnPaths.math;
    }
    if (title === 'Data Structures & Algorithms') {
      categoryBasePath = learnPaths.dsa;
    }
    if (title === 'AI & Machine Learning') {
      categoryBasePath = learnPaths.ai;
    }
